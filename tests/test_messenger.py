...
from src.data_manager import CONVERSATION_DIR, save_conversation, load_conversation, generate_id
...
def test_send_without_id(mock_print):
    ...
    assert messenger.conversation_history[0] == {"sender": "assistant", "text": test_message}
...
def test_receive_without_id(mock_input):
    ...
    # Check that the message was added to conversation history
    assert len(messenger.conversation_history) == 1
    assert messenger.conversation_history[0] == {"sender": "user", "text": expected_input}
